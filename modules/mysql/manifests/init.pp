class mysql 
{
    ...
    { 
        "create-default-db":
            ...
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'create database `ebibliothek`;'",
    ...
    { 
        "grant-default-db":
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'grant all on `ebibliothek`.* to `root@localhost`;'",
